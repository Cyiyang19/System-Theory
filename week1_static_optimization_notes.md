...
The source's two-dimensional example uses $Q=\left[\begin{smallmatrix}1&1\\\\1&2\end{smallmatrix}\right]$ and $S=(0,1)^T$. Its stationary point is $(1,-1)^T$, with $L^{\ast}=-1/2$ (PDF P12–P13). This gives a concrete interpretation of the contour plot: a **contour** is the set of points with one fixed value of $L$. Along a contour, $dL\approx L_u^Tdu=0$, so the gradient is perpendicular to its tangent direction. $L_u$ points toward increasing cost, and $-L_u$ gives the direction of steepest local decrease in the usual Euclidean scale.（等高線上數值不變；梯度垂直等高線，負梯度指向下降最快的方向。）
...
1. $L_{(x_1,x_2)}=(2x_1-x_2+3,\,-x_1+2x_2)^T$. Solving both entries equal to zero gives $(x_1^{\ast},x_2^{\ast})=(-2,-1)$. The Hessian is $\left[\begin{smallmatrix}2&-1\\\\-1&2\end{smallmatrix}\right]$, with eigenvalues $1$ and $3$, hence positive definite. Therefore it is the unique global minimum of this quadratic, with $L^{\ast}=-3$.
...
5. With $H=\tfrac12z^Tz+\lambda^T(Az-b)$, stationarity gives $z=-A^T\lambda$ and feasibility gives $-AA^T\lambda=b$. Since $AA^T=\left[\begin{smallmatrix}14&4\\\\4&14\end{smallmatrix}\right]$, $\lambda=\left(1/90,-13/45\right)^T$ and $z^{\ast}=\left(23/90,5/9,-79/90\right)^T$. Direct substitution verifies both equations. Its distance from the origin is $\|z^{\ast}\|=\sqrt{103/90}$. The squared norm is strictly convex and the constraints are linear, so this is the unique global minimum.

...
Setting $L_u=0$ gives $u^{\ast}=4$. Since the curvature is positive, this is a minimum, and $L^{\ast}=L(4)=3$.
...
The only stationary point is $(0,0)$, and both curvature directions are positive, so it is a strict minimum with $L^{\ast}=0$.
...
$$
u^{\ast}=-Q^{-1}S,\qquad L(u^{\ast})=-\frac12S^TQ^{-1}S.
$$
...
$$
u^{\ast}=\begin{bmatrix}2\\\\-2\end{bmatrix},\qquad
L^{\ast}=\frac12(2\cdot 2^2+4\cdot(-2)^2)-4(2)+8(-2)=-12.
$$

Since $Q\succ0$, this quadratic has a unique global minimum.

The source's two-dimensional example uses the symmetric matrix $Q$ with rows $(1,1)$ and $(1,2)$ and $S=(0,1)^T$. Its stationary point is $(1,-1)^T$, with $L^{\ast}=-1/2$ (PDF P12–P13). This gives a concrete interpretation of the contour plot: a **contour** is the set of points with one fixed value of $L$. Along a contour, $dL\approx L_u^Tdu=0$, so the gradient is perpendicular to its tangent direction. $L_u$ points toward increasing cost, and $-L_u$ gives the direction of steepest local decrease in the usual Euclidean scale.（等高線上數值不變；梯度垂直等高線，負梯度指向下降最快的方向。）
...
The first two give $x=u$; the third then gives $x^{\ast}=u^{\ast}=1$ and $\lambda^{\ast}=-2$. Thus $L^{\ast}=2$. Substitution along the feasible line, $x=2-u$, gives
...
In worked example E, feasible changes satisfy $dx=-du$, so the tangent direction is $(-1,1)^T$. Because $H_{zz}=\mathrm{diag}(2,2)$, its curvature along that direction is $(-1,1)H_{zz}(-1,1)^T=4>0$, matching the direct substitution.
...
$$dL^{\ast}\approx-\lambda^{*T}d\delta.$$

For worked example E, requiring $x+u-2=\delta$ yields $L^{\ast}(\delta)=(2+\delta)^2/2$. Its derivative at $\delta=0$ is $2=-\lambda^{\ast}$. This is the **shadow price** idea: it tells how much the best achievable cost changes when the requirement shifts slightly. The sign depends on how the constraint is written; if a parameter is added *inside* $f(x,u,c)=0$, then the chain rule gives $dL^{\ast}/dc=\lambda^{*T}f_c$.（乘子的正負不能脫離限制式寫法單獨解讀。）

**A source example:** PDF P18–P20 fixes $x=3$ for $L=\tfrac12x^2+xu+u^2+u$. Solving its Hamiltonian equations gives $(x^{\ast},u^{\ast},\lambda^{\ast})=(3,-2,-1)$, $L^{\ast}=1/2$, and positive feasible curvature $2$. This illustrates that the constrained optimum need not be the unconstrained optimum.
...
**Quadratic problem.** For symmetric $Q$, $L=\tfrac12u^TQu+S^Tu$ has $L_u=Qu+S$ and $L_{uu}=Q$. When $Q$ is invertible, $u^{\ast}=-Q^{-1}S$. It is a minimum when $Q\succ0$. For the two-dimensional case, $|Q|$ in the source is $\det Q$.

**Equality constraint.** In $\min L(x,u)$ subject to $f(x,u)=0$, feasible changes obey $f_xdx+f_udu=0$. When $f_x$ is nonsingular, the first-order condition is $L_u-f_u^Tf_x^{-T}L_x=0$. Define $H=L+\lambda^Tf$; the equivalent candidate equations are $H_\lambda=f=0$, $H_x=L_x+f_x^T\lambda=0$, and $H_u=L_u+f_u^T\lambda=0$. Then check curvature along feasible directions. With the convention $f=\delta$, $dL^{\ast}/d\delta=-\lambda^{\ast}$ locally.
...
3. **Constrained area** (adapted from Problem 1.2-5(a), PDF P26). A rectangle has positive side lengths $x,y$ and perimeter $p>0$. Maximize its area $xy$ subject to $2x+2y-p=0$. Find $x^{\ast},y^{\ast}$, the maximum area, and the multiplier using $H=xy+\lambda(2x+2y-p)$.
...
5. **Two equality constraints** (adapted from Problem 1.2-9, PDF P27). Find the point $z=(x,y,z_3)^T$ nearest the origin that satisfies $3x+2y+z_3=1$ and $x+2y-3z_3=4$. Hint: minimize $\tfrac12z^Tz$ subject to $Az=b$, where $A$ has rows $(3,2,1)$ and $(1,2,-3)$, and $b=(1,4)^T$.

## 9. Answers to practice problems

1. $L_{(x_1,x_2)}=(2x_1-x_2+3,\,-x_1+2x_2)^T$. Solving both entries equal to zero gives $(x_1^{\ast},x_2^{\ast})=(-2,-1)$. The Hessian is the symmetric matrix with rows $(2,-1)$ and $(-1,2)$, with eigenvalues $1$ and $3$, hence positive definite. Therefore it is the unique global minimum of this quadratic, with $L^{\ast}=-3$.
2. $\nabla g=(2x,4y^3)^T$, so the origin is stationary. $\nabla^2g(0,0)=\mathrm{diag}(2,0)$ is singular and the second-order test is inconclusive. Directly, $x^2+y^4\ge0$ with equality only at $(0,0)$, so the origin is a strict global minimum.
...
4. $H=x^2+u^2+\lambda(x+u-2)$, so $H_x=2x+\lambda=0$, $H_u=2u+\lambda=0$, and $H_\lambda=x+u-2=0$. Hence $(x^{\ast},u^{\ast},\lambda^{\ast})=(1,1,-2)$ and $L^{\ast}=2$. $(0,0)$ violates $x+u-2=0$, so its smaller unconstrained cost is irrelevant.
5. With $H=\tfrac12z^Tz+\lambda^T(Az-b)$, stationarity gives $z=-A^T\lambda$ and feasibility gives $-AA^T\lambda=b$. Since $AA^T$ has rows $(14,4)$ and $(4,14)$, $\lambda=\left(1/90,-13/45\right)^T$ and $z^{\ast}=\left(23/90,5/9,-79/90\right)^T$. Direct substitution verifies both equations. Its distance from the origin is $\|z^{\ast}\|=\sqrt{103/90}$. The squared norm is strictly convex and the constraints are linear, so this is the unique global minimum.
